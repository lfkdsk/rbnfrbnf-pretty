...
def plot_graph(nodes: List[Node]):
    ...
    def attrs(node: Node):
        if isinstance(node, NamedTerminal):
            return id(node), 'orange', repr(node), 4
        if isinstance(node, NonTerminalEnd):
            return id(node), 'pink', repr(node), 4
        if isinstance(node, UnnamedTerminal):
            return id(node), 'blue', repr(node), 3
        if isinstance(node, Identified):
            return id(node), 'red', repr(node), 6
        if isinstance(node, SubRoutine):
            return id(node), 'teal', repr(node), 5
        if isinstance(node, TerminalEnd):
            return id(node), 'yellow', repr(node), 3
    ...
    for node in nodes:
        for parent in node.parents:
            json_links.append(make_link(start=id(node), end=id(parent), value=3))

    return {
        'width': 1200, 'height': 1000, 'data': {
            'links': json_links, 'nodes': json_nodes
        }
    }
